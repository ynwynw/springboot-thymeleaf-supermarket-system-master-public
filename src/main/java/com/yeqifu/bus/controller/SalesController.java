...
public class SalesController {
     ...
     * @return
     */
    @RequestMapping("loadAlldelSales")
    public DataGridView loadAlldelSales(SalesVo salesVo){
        IPage<Sales> page = new Page<>(salesVo.getPage(),salesVo.getLimit());
        QueryWrapper<Sales> queryWrapper = new QueryWrapper<Sales>();
        //根据客户进行模糊查询
        queryWrapper.eq(salesVo.getCustomerid()!=null&&salesVo.getCustomerid()!=0,"customerid",salesVo.getCustomerid());
    ...
    public ResultObj addSales(SalesVo salesVo){
        try {
           ...
           Customer customer= customerService.getById(salesVo.getCustomerid());
           if(customer.getStu().equals("会员")){
               salesVo.setSaleprice(salesVo.getSaleprice()*0.9);
           }
            //设置操作人
     ...
     * @return
     */
    @RequestMapping("nodeleteSales")
    public ResultObj nodeleteSales(Integer id){
        try {
